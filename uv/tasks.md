...
settings = Settings()
```
If you try to run it with `uv run a_script.py` now, it will fail because pydantic-settings (a library required to run this module) is not installed. Add it to script requirements (uv add --script ...) and try running the script again.
